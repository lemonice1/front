...
window.addEventListener('DOMContentLoaded', function () {
    ...
    function customFile() {
        ...
        if (location.protocol === 'file:') {
            guiplayIconCSS = 'https://y.cenguigui.cn/js/gui-MusicPlayer/icon/guiplayIcon.min.css'
            MusicPlayerCSS = 'https://y.cenguigui.cn/js/gui-MusicPlayer/css/gui-MusicPlayer.min.css'
        ...
        function allPlayerFeatures() {
            ...
            const clickControl = () => {
                ...
                const togglePlayback = () => {
                    const domLength = MusicPlayer.getElementsByClassName('gui-bePlaying').length
                    if (domLength > 0) {
                        displayPopup('音乐已暂停')
                        pauseMusic()
                        removebePlaying()
                ...
                const loadguiPlayer = async (dom, val, key) => {
                    const guiStyle = window.getComputedStyle(dom)
                    await setTimeoutPromise(1000)
                    if (guiStyle.getPropertyValue(val) === key) {
                        displayPopup('播放器加载中...')
                ...
                const playBackAndForth = async () => {
                    try {
                        ...
                        checkSongsItemLength().then(async waitTime => {
                            ...
                            let songsItem = MusicPlayer.querySelectorAll('.gui-songsItem')
                            ...
                            const updateSong = index => {
                                ...
                                songsItem.forEach((ele, i) => {
                                    ...
                                    const filteredinExecution = Array.from(songsItem).filter(ele => ele.classList.contains('gui-inExecution'))
                                    eleInExecution = filteredinExecution
                                    ele.querySelector('.gui-songListSongPictures .gui-songIcon').classList.remove('icon-zantingtingzhi')
                                })
                                ...
                                guiMusicAudio.src = itemUrl
                                musicPicture.src = itemPic
                                musicPicture.alt = itemName
                                songName.textContent = itemName
                                singer.textContent = itemAuto

                                const guiSongIcon = eleInExecution[0].querySelector('.gui-songListSongPictures .gui-songIcon')
                                guiSongIcon.classList.add('icon-zantingtingzhi')
                                        ...
                                        .then(res => {
                                            ...
                                            if (res.code === 200) {
                                                ...
                                                lyricsArray.forEach(lyric => {
                                                    ...
                                                    lisEle.classList.add('gui-ly')
                                                    lisEle.textContent = lyric.text
                                                    guiAllLyri.appendChild(lisEle)
                                                })
                                                ...
                                                function updateLyricDisplay() {
                                                    ...
                                                    for (let i = 0; i < lyricsArray.length; i++) {
                                                        const lisEle = guiAllLyri.children[i]
                                                        if (lisEle) {
                                                            lisEle.classList.remove('gui-textShow')
                                                        }
                                                    }
                                                    ...
                                                    const lisEle = guiAllLyri.children[currentLyricIndex]
                                                    if (lisEle) {
                                                        lisEle.classList.add('gui-textShow')
                                                    }
                                                }

                                                guiMusicAudio.removeEventListener('timeupdate', updateLyricDisplay)

                                                guiMusicAudio.addEventListener('timeupdate', updateLyricDisplay)
                ...
                const elementDetection = setTimeout(() => {
                    const lis = [...listOfSongs.querySelectorAll('.gui-songsItem')]
                ...
                const slide = e => {
                    ...
                    guiMusicAudio.currentTime = newTime
                }
                ...
                playlistBtn.addEventListener('click', () => {
                    const showSong = MusicPlayer.getElementsByClassName('gui-outsideSongListShow').length
                    showSong ? outsideSongList.classList.remove('gui-outsideSongListShow') : outsideSongList.classList.add('gui-outsideSongListShow')
                })
                ...
                const goThroughShowAndLeaveHidden = () => {
                    if (!throughDisplayDiv) {
                        ...
                        throughDisplayDiv.classList.add('gui-throughDisplay')
            ...
            switchPlayerFun()
            guiMusicAudio.remove()
        }
    }
    const message = '本站自豪的采用笒鬼鬼播放器'
